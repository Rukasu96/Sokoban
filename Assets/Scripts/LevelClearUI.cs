...
public class LevelClearUI : MonoBehaviour
{
    ...
    private async void startAnim()
    {
        var tasks = new List<Task>
        {
            ...
            levelClearTransform.DOScale(1.2f, 1f).AsyncWaitForCompletion(),
            levelClearTransform.DOLocalMoveY(238, 1f).SetEase(Ease.Linear).AsyncWaitForCompletion(),
    ...
    private void PlayRatingAnimation()
    {
        ...
        for (int i = 0; i < ratingCalculator.GetRating(); i++)
        {
            ...
            RatingSequence.Append(stars[i].DOScale(1.4f, 0.5f).SetLoops(2, LoopType.Yoyo));
        }
